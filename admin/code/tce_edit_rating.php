...
if (isset($changecategory) && $changecategory > 0 || ! isset($testlog_id) || empty($testlog_id)) {
    $sql = 'SELECT test_id, test_score_right, test_score_wrong, test_score_unanswered, testlog_id, testlog_score, testlog_answer_text, testlog_comment, question_id, question_description, question_difficulty, question_explanation,  question_spring_id
...
if ($sql !== '' && $sql !== '0') {
    if ($r = F_db_query($sql, $db)) {
        if ($m = F_db_fetch_array($r)) {
            ...
            $test_id = $m['test_id'];
            $question_id = $m['question_id']; // Added question_id
            $question_spring_id = $m['question_spring_id'];
            ...
            $testlog_id = '';
            $test_id = ''; // Ensure test_id is also cleared
...
echo '<span class="label">&nbsp;</span>' . K_NEWLINE;
echo '<span class="formw">' . K_NEWLINE;
echo '<button type="button" id="autoEvaluateBtn" class="xmlbutton" title="' . $l['h_auto_evaluate'] . '">';
echo $l['w_auto_evaluate']; // Translated label for "Evaluate Automatically"
echo '</button>' . K_NEWLINE;
echo '<span id="evaluationResult" style="color: #333; font-weight: bold; margin-left: 10px;"></span>' . K_NEWLINE;
echo '</span>' . K_NEWLINE;

echo '</div>' . K_NEWLINE;

// Add a container to show evaluation results
echo '<div id="evaluationResult" style="margin-top: 15px; color: #333; font-weight: bold;"></div>' . K_NEWLINE;
...
<script>
    document.getElementById('autoEvaluateBtn').addEventListener('click', function() {
        ...
        evaluationResult.innerHTML = 'Evaluating...'; // Show loading message

        // Data to send to the evaluation API
        const payload = {
            question_id: <?php echo json_encode($question_id); ?>,
            user_answer: <?php echo json_encode($answer); ?>,
            question_spring_id: <?php echo json_encode($question_spring_id); ?>
        };

        console.log('Payload:', payload); // Debugging payload data


        // Make the AJAX call to the evaluation API
        fetch('quizai_evaluate_answer.php', {
                method: 'POST',
                headers: {
            ...
            .catch(error => {
                ...
                evaluationResult.innerHTML = `Error: ${error.message || 'An unexpected error occurred.'}`;
            });
    });
</script>
<!-- <script> test with dummy data for appearance


        // JavaScript to Test Button Functionality with Dummy Data
        document.getElementById('autoEvaluateBtn').addEventListener('click', function () {
            const evaluationResult = document.getElementById('evaluationResult');
            evaluationResult.innerHTML = 'Evaluating...'; // Show loading message

            // Dummy data payload for testing
            const payload = {
                question: "Write a short essay on climate change.",
                user_answer: "Climate change refers to the long-term alteration of temperature and typical weather patterns in a place.",
                explanation: "This is an essay question about climate change."
            };
            ...
            setTimeout(() => {
                ...
                const dummyResponse = {
                    score: 85,
                    feedback: "The answer is accurate but lacks depth and examples."
                };

                // Display the simulated response
                evaluationResult.innerHTML = `<br> Score: ${dummyResponse.score} <br> Feedback: ${dummyResponse.feedback}`;
